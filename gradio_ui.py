...
def process_vision(image):
    if image is None: return None, "No image", "No events"
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # format the string
            out_str = " | ".join([f"{k.capitalize()}: {v}" for k, v in counts_dict.items()])
            if not out_str: out_str = "No objects detected"
            
            # get events
            events_res = requests.get("http://127.0.0.1:8003/api/v1/events").json()
            events_text = "\n".join([f"[{e['timestamp']}] {e['description']}" for e in events_res])
...
def get_timeline():
    try:
        ...
        return "\n\n".join([f"{e['timestamp']}: {e['caption']}" for e in res.get("entries", [])])
    except Exception as e:
        return f"Error: {e}"

def track_usage(app_name, mins):
    try:
        requests.post("http://127.0.0.1:8002/api/v1/track", json={"app_name": app_name, "minutes": int(mins)})
        alerts = requests.get("http://127.0.0.1:8002/api/v1/alerts").json()
        return f"Logged {mins} mins for {app_name}. Total Alerts: {len(alerts)}\n" + "\n".join([a['message'] for a in alerts])
    except Exception as e:
        return f"Error: {e}"

def start_focus(app_name, mins):
    try:
        res = requests.post("http://127.0.0.1:8002/api/v1/focus", json={"app_to_block": app_name, "duration_minutes": int(mins)}).json()
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tab("Visual Intelligence App 👁️"):
        gr.Markdown("Real-time YOLOv8 object detection on your uploaded image or webcam.")
        with gr.Row():
            vision_in = gr.Image(sources=["webcam", "upload"], label="Input Camera/Image")
            with gr.Column():
                vision_out_img = gr.Image(label="Annotated Output")
                vision_out_stats = gr.Textbox(label="Detection Counts")
                vision_out_events = gr.Textbox(label="Event Engine Logs (e.g. Queue Detected)")
        
        vision_in.change(process_vision, inputs=vision_in, outputs=[vision_out_img, vision_out_stats, vision_out_events])
    ...
    with gr.Tab("Doomscroll Breaker App 📵"):
        gr.Markdown("Track usage of addictive apps, receive alerts, and initiate focus sessions.")
        with gr.Row():
            with gr.Column():
