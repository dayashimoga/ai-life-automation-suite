...
def init_identity_db():
    if not os.path.exists(DB_PATH):
        ...
        with open(os.path.join(DB_PATH, "README.md"), "w") as f:
            f.write(
                "Drop images of authorized personnel in this directory to seed the Authorized Users DB for DeepFace."
            )
...
def analyze_identities(frame_np) -> List[str]:
    ...
    try:
        ...
        dfs = DeepFace.find(
            img_path=frame_np, db_path=DB_PATH, enforce_detection=True, silent=True
        )
