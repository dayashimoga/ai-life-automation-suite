...
class SpatialAnalyticsEngine:
    ...
    def process_frame(
        self, frame_np: np.ndarray, detections: List[Detection]
    ) -> np.ndarray:
        if frame_np is None:
            return None

        if (
            self.heatmap_layer is None
            or self.heatmap_layer.shape[:2] != frame_np.shape[:2]
        ):
            ...
            if det.label == "person":
                ...
                if has_helmet:
                    cv2.putText(
                        frame_np,
                        "HELMET",
                        (int(det.bbox.x), int(det.bbox.y - 30)),
        ...
        if np.any(mask):
            frame_np[mask] = cv2.addWeighted(
                frame_np[mask], 0.6, heatmap_color[mask], 0.4, 0
            )
        ...
        cv2.putText(
            frame_np,
            f"[TELEMETRY] LINE IN: {self.entry_count} | LINE OUT: {self.exit_count}",
            (10, center_y + 25),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            (0, 255, 0),
