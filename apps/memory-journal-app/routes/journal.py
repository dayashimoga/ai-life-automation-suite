...
async def process_video_entry(file: UploadFile = File(...)):
    ...
    if ret:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                resp = await client.post("http://127.0.0.1:5002/api/v1/vision/process", files=files, timeout=10.0)
    ...
    entry = JournalEntry(
        id=str(uuid4()),
        filename=file.filename or "video_keyframe.jpg", # Use original filename or a default
...
async def semantic_search(query: str, top_k: int = 10):
    ...
    docs = [
        {"id": e.id, "caption": e.caption, "tags": e.tags, "filename": e.filename, "timestamp": str(e.timestamp)}
        for e in db_entries
    ]
    ...
    return {
        "query": query,
        "results": [
            {"entry": doc, "relevance_score": round(score, 4)}
