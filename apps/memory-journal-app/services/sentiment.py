...
NEGATIVE_TERMS = {
    ...
    "exhausted": 3, "burnout": 3, "overwhelmed": 3, "hopeless": 3, "can't cope": 3,
    "breakdown": 3, "depressed": 3, "crying": 2, "anxiety": 2, "panic": 2,
    # Medium weight
    "tired": 1.5, "stressed": 1.5, "anxious": 1.5, "worried": 1.5, "lonely": 1.5,
    "frustrated": 1, "angry": 1, "sad": 1, "bored": 1, "drained": 1.5,
    # Low weight
    "bad": 0.5, "hard": 0.5, "tough": 0.5, "difficult": 0.5, "ugh": 0.5,
}

POSITIVE_TERMS = {
    # High weight — strong wellness signals
    "grateful": 2, "productive": 2, "energized": 2, "happy": 2, "thriving": 2,
    "accomplished": 2, "excited": 2, "inspired": 2, "motivated": 2, "joyful": 2,
    # Medium weight
    "good": 1, "great": 1, "calm": 1, "relaxed": 1, "peaceful": 1,
    "focused": 1, "confident": 1, "healthy": 1, "rested": 1, "bright": 0.5,
}
...
def _score_text(text: str) -> Dict[str, float]:
    ...
    neg_score = sum(
        weight for term, weight in NEGATIVE_TERMS.items()
        if re.search(r"\b" + re.escape(term) + r"\b", text_lower)
    )
    pos_score = sum(
        weight for term, weight in POSITIVE_TERMS.items()
        if re.search(r"\b" + re.escape(term) + r"\b", text_lower)
    )
...
def analyze_entries(entries: List[Dict]) -> Dict:
        ...
        if scores["negative"] > 2:
            signals.append(
                {"entry": text[:50] + "..." if len(text) > 50 else text,
                 "signal": "high_stress", "score": scores["negative"]}
            )
    ...
    if burnout_risk > 0.65:
        mood_trend, recommendation = "declining", "🚨 Signs of burnout detected. Consider a digital detox day and reach out to someone you trust."
    elif burnout_risk > 0.40:
        mood_trend, recommendation = "stressed", "⚠️ Elevated stress in your journal. Try a 5-minute breathing exercise before bed tonight."
    elif burnout_risk > 0.20:
        mood_trend, recommendation = "mixed", "😐 Some mixed emotions lately. A short walk outside could reset your mood."
    else:
        mood_trend, recommendation = "positive", "✅ Your journal reflects a positive mindset! Keep the momentum going."
