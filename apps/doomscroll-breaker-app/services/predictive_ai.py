...
class AdaptivePredictor:
    ...
    def _get_historical_risk(self, hour: int, day: int) -> float:
        ...
        try:
            ...
            for row in rows[1:]:
                ewma = (
                    self.EWMA_ALPHA * row["risk_score"] + (1 - self.EWMA_ALPHA) * ewma
                )
            return ewma
        except Exception:
            return self.BASE_RISK.get(hour, 0.5)
    ...
    def _log_session(
        self, app_name: str, hour: int, day: int, risk: float, was_doom: bool
    ):
    ...
    def get_usage_analytics(self) -> dict:
        ...
        try:
            db = _get_db()
            total = db.execute("SELECT COUNT(*) as cnt FROM usage_log").fetchone()[
                "cnt"
            ]
            doom_count = db.execute(
                "SELECT COUNT(*) as cnt FROM usage_log WHERE was_doomscroll = 1"
            ).fetchone()["cnt"]
            avg_risk = (
                db.execute("SELECT AVG(risk_score) as avg FROM usage_log").fetchone()[
                    "avg"
                ]
                or 0
            )
            ...
            return {
                ...
                "top_apps": [
                    {"app": r["app_name"], "sessions": r["cnt"]} for r in top_apps
                ],
                "risk_hotspots": [
                    {"hour": r["hour"], "avg_risk": round(r["avg_risk"], 3)}
                    for r in hotspot_hours
                ],
            }
        except Exception:
